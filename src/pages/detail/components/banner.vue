<template>
	<div>
		...
		<face-animation>
			<CommonGallary  
				v-show="this.$store.state.isShowGallary"
				@close="handelGallaryClick"
...
<script>
	import CommonGallary from "common/gallery.vue";
	import FaceAnimation from "@/common/faceAnimation.vue";
	export default{
		components:{
			CommonGallary,
	    	FaceAnimation
		...
		methods:{
			handelBannerClick(){
				this.$store.commit('changeIsShowGallary')
			},
			handelGallaryClick(){
				this.$store.commit('changeIsShowGallary')
			}
